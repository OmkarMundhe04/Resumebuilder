...
/**
 * Universal Custom Cursor Primitive
 * Inspired by the Portfolio benchmark.
 * Provides subtle interactive follower ring + precision center dot.
 * Automatically disabled on touch devices and when prefers-reduced-motion is active.
 */
const CustomCursor = () => {
  const [isVisible, setIsVisible] = useState(false);
  const [cursorType, setCursorType] = useState('default'); // 'default', 'link', 'view', 'button', 'drag'
  ...
  useEffect(() => {
    ...
    const handleElementHover = (e) => {
      const target = e.target.closest('[data-cursor], a, button, input, textarea, .card-hover, .clickable');
      if (!target) {
  ...
  return (
    <div className="custom-cursor-container no-print" style={{ pointerEvents: 'none', position: 'fixed', inset: 0, zIndex: 99999 }}>
      {/* Outer Follower Ring */}
      <motion.div
