...
const SiliconValley = ({ resume }) => {
  ...
  const fontFamily = formatting?.fontFamily || 'Inter, system-ui, sans-serif';

  return (
    <div 
      className="canonical-resume silicon-valley"
      style={{
        fontFamily,
        color: '#0f172a',
    ...
    >
      {/* Header */}
      <header data-section="personal" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', borderBottom: `2px solid ${accentColor}`, paddingBottom: '12px' }}>
        <div>
          ...
          <div style={{ fontSize: '8.5pt', color: '#64748b', marginTop: '4px', display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
            {personal?.email && <span>✉ <ResumeLink href={personal.email} style={{ color: '#0f172a' }} /></span>}
            {personal?.phone && <span>📱 <ResumeLink href={personal.phone} style={{ color: '#0f172a' }} /></span>}
        ...
        {personal?.showPhoto && personal?.photoUrl && (
          <img 
            ...
            style={{ width: '60px', height: '60px', borderRadius: '50%', objectFit: 'cover', border: `2px solid ${accentColor}` }}
          />
        )}
      </header>
          ...
          case 'skills':
            ...
            return (
              <section key="skills" data-section="skills" className="resume-section" style={{ marginBottom: '14px', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                <h2 style={{ fontSize: '9.5pt', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: accentColor, margin: '0 0 6px 0', breakAfter: 'avoid', pageBreakAfter: 'avoid' }}>
                  Technical Skills
                </h2>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
                  {skills.map((s, idx) => {
                    ...
                    return (
                      <span key={s.id || idx} style={{ fontSize: '8pt', padding: '2px 8px', background: 'rgba(37, 99, 235, 0.08)', color: '#1e40af', borderRadius: '12px', fontWeight: 600 }}>
          ...
          case 'experience':
            ...
            return (
              <section key="experience" data-section="experience" className="resume-section" style={{ marginBottom: '16px' }}>
                <h2 style={{ fontSize: '9.5pt', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: accentColor, margin: '0 0 8px 0', breakAfter: 'avoid', pageBreakAfter: 'avoid' }}>
                  Work Experience
                </h2>
                      ...
                      <div key={exp.id || idx} className="experience-item" style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap' }}>
                          <div>
                            ...
                            <span style={{ fontSize: '9pt', color: '#475569' }}> at <strong>{exp.company}</strong></span>
                        ...
                        {validBullets.length > 0 && (
                          <ul style={{ margin: '4px 0 0 16px', padding: 0, fontSize: '9.5pt', color: '#334155' }}>
                ...
                {projects.map((proj, idx) => {
                  ...
                  return (
                    <div key={proj.id || idx} className="project-item" style={{ marginBottom: '6px', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                      ...
                      {techStr && (
                        <div style={{ fontSize: '8pt', color: '#64748b', margin: '1px 0' }}>
                          Tech: {techStr}
                        </div>
                      )}
                      {proj.description && <div style={{ fontSize: '9pt', color: '#475569' }}>{proj.description}</div>}
                      {validBullets.length > 0 && (
