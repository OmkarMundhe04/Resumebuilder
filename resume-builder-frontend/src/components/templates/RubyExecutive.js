...
const RubyExecutive = ({ resume }) => {
  ...
  const accentColor = formatting?.accentColor || '#991b1b';
  const fontFamily = formatting?.fontFamily || 'Cinzel, Georgia, "Times New Roman", serif';

  return (
    <div 
      className="canonical-resume ruby-executive"
      style={{
        fontFamily,
        color: '#111827',
    ...
    >
      {/* Header */}
      <header data-section="personal" style={{ textAlign: 'center', borderBottom: `2px solid ${accentColor}`, paddingBottom: '14px', marginBottom: '16px' }}>
        ...
        <h1 style={{ fontSize: '22pt', fontWeight: 700, margin: 0, color: accentColor, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          {personal?.fullName || 'Your Name'}
        </h1>
        {personal?.title && (
          <div style={{ fontSize: '11pt', fontWeight: 600, color: '#374151', fontStyle: 'italic', marginTop: '2px' }}>
            {personal.title}
          </div>
        )}
        <div style={{ fontSize: '8.5pt', color: '#4b5563', marginTop: '6px', display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '10px', fontFamily: 'sans-serif' }}>
          ...
          case 'summary':
            ...
            return (
              <section key="summary" data-section="summary" className="resume-section" style={{ marginBottom: '14px' }}>
                <h2 style={{ fontSize: '9.5pt', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: accentColor, borderBottom: '1px solid #fee2e2', paddingBottom: '2px', marginBottom: '4px', breakAfter: 'avoid', pageBreakAfter: 'avoid' }}>
                  Leadership Statement
                </h2>
                <p style={{ margin: 0, color: '#374151', fontSize: '9.5pt', lineHeight: 1.5, textAlign: 'justify', fontFamily: 'sans-serif' }}>
                  {personal.summary}
                </p>
              </section>
            );
          ...
          case 'experience':
            ...
            return (
              <section key="experience" data-section="experience" className="resume-section" style={{ marginBottom: '16px' }}>
                <h2 style={{ fontSize: '9.5pt', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: accentColor, borderBottom: '1px solid #fee2e2', paddingBottom: '2px', marginBottom: '8px', breakAfter: 'avoid', pageBreakAfter: 'avoid' }}>
                      ...
                      <div key={exp.id || idx} className="experience-item" style={{ breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap' }}>
                          <div>
                            ...
                            <span style={{ fontSize: '9.5pt', color: '#4b5563' }}> — {exp.company}</span>
          ...
          case 'skills':
            ...
            return (
              <section key="skills" data-section="skills" className="resume-section" style={{ marginBottom: '14px', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                <h2 style={{ fontSize: '9.5pt', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: accentColor, borderBottom: '1px solid #fee2e2', paddingBottom: '2px', marginBottom: '6px', breakAfter: 'avoid', pageBreakAfter: 'avoid' }}>
                  Core Competencies
                </h2>
                <div style={{ fontSize: '9.5pt', color: '#374151', lineHeight: 1.6, fontFamily: 'sans-serif' }}>
                  {skills.map(s => {
            ...
            return (
              <section key="projects" data-section="projects" className="resume-section" style={{ marginBottom: '14px' }}>
                ...
                {projects.map((proj, idx) => {
                  ...
                  return (
                    <div key={proj.id || idx} className="project-item" style={{ marginBottom: '6px', fontFamily: 'sans-serif', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap' }}>
                        <div>
                          <strong style={{ fontSize: '9.5pt', color: '#111827' }}>{proj.name}</strong>
                          {proj.role && <span style={{ fontSize: '9pt', color: '#4b5563' }}> ({proj.role})</span>}
                        </div>
                        {proj.link && (
                          <ResumeLink 
                            href={proj.link} 
                      ...
                      {techStr && <div style={{ fontSize: '8.5pt', color: '#6b7280', margin: '1px 0' }}>Tech: {techStr}</div>}
                      {proj.description && <div style={{ fontSize: '9pt', color: '#4b5563' }}>{proj.description}</div>}
                      {validBullets.length > 0 && (
                        <ul style={{ margin: '2px 0 0 16px', padding: 0, fontSize: '9.5pt', color: '#374151' }}>
                          {validBullets.map((b, bIdx) => (
              ...
              <section key="education" data-section="education" className="resume-section" style={{ marginBottom: '14px' }}>
                ...
                {education.map((edu, idx) => (
                  <div key={edu.id || idx} className="education-item" style={{ marginBottom: '4px', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '9.5pt', fontFamily: 'sans-serif' }}>
                      <div>
                        <strong>{edu.degree}</strong>, {edu.institution}
                        {edu.location && <span style={{ color: '#6b7280', fontSize: '8.5pt' }}> ({edu.location})</span>}
                        {edu.gpa && <span style={{ color: accentColor, fontSize: '8.5pt' }}> | GPA: {edu.gpa}</span>}
                      </div>
                      <span style={{ color: '#6b7280', fontSize: '8.5pt' }}>{edu.startDate} — {edu.endDate}</span>
                    </div>
                    {edu.coursework && (Array.isArray(edu.coursework) ? edu.coursework.length > 0 : edu.coursework) && (
                      <div style={{ fontSize: '8.5pt', color: '#6b7280', fontFamily: 'sans-serif' }}>
                    ...
                    {edu.honors && (Array.isArray(edu.honors) ? edu.honors.length > 0 : edu.honors) && (
                      <div style={{ fontSize: '8.5pt', color: accentColor, fontFamily: 'sans-serif' }}>
            ...
            return (
              <section key="certifications" data-section="certifications" className="resume-section" style={{ marginBottom: '14px' }}>
                ...
                {certifications.map((c, idx) => {
                  ...
                  return (
                    <div key={c.id || idx} className="certification-item" style={{ fontSize: '9pt', color: '#374151', marginBottom: '2px', fontFamily: 'sans-serif', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                      <strong>{c.name}</strong> — {c.issuer} {c.issueDate ? `(${c.issueDate}${c.expiryDate ? ` - ${c.expiryDate}` : ''})` : ''}
                      {c.credentialId && <span style={{ color: '#6b7280', fontSize: '8.5pt' }}> [ID: {c.credentialId}]</span>}
            ...
            return (
              <section key="publications" data-section="publications" className="resume-section" style={{ marginBottom: '14px' }}>
                ...
                {publications.map((p, idx) => (
                  <div key={p.id || idx} className="publication-item" style={{ fontSize: '9pt', color: '#374151', marginBottom: '2px', fontFamily: 'sans-serif', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
          ...
          case 'awards':
            ...
            return (
              <section key="awards" data-section="awards" className="resume-section" style={{ marginBottom: '14px' }}>
                ...
                {awards.map((a, idx) => (
                  <div key={a.id || idx} className="award-item" style={{ fontSize: '9pt', color: '#374151', marginBottom: '2px', fontFamily: 'sans-serif', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
          ...
          case 'volunteer':
            ...
            return (
              <section key="volunteer" data-section="volunteer" className="resume-section" style={{ marginBottom: '14px' }}>
                ...
                {volunteer.map((v, idx) => (
                  <div key={v.id || idx} className="volunteer-item" style={{ fontSize: '9pt', color: '#374151', marginBottom: '2px', fontFamily: 'sans-serif', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
              ...
              <React.Fragment key="custom">
                {customSections.map((cs, idx) => (
                  <section key={cs.id || idx} data-section="custom" className="resume-section" style={{ marginBottom: '14px' }}>
                    ...
                    <p style={{ margin: 0, fontSize: '9.5pt', color: '#374151', fontFamily: 'sans-serif', whiteSpace: 'pre-line' }}>{cs.content}</p>
                  </section>
                ))}
              </React.Fragment>
            );
