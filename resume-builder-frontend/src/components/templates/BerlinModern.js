...
const BerlinModern = ({ resume }) => {
  ...
  const fontFamily = formatting?.fontFamily || '"Space Grotesk", Inter, sans-serif';

  return (
    <div 
      className="canonical-resume berlin-modern"
      style={{
        fontFamily,
        color: '#18181b',
    ...
    >
      {/* Split Modern Header */}
      <header data-section="personal" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#f8fafc', padding: '16px 20px', borderRadius: '6px', borderLeft: `4px solid ${accentColor}`, marginBottom: '18px' }}>
        <div>
          <h1 style={{ fontSize: '20pt', fontWeight: 800, margin: 0, color: '#0f172a', letterSpacing: '-0.02em' }}>
            {personal?.fullName || 'Your Name'}
          </h1>
          {personal?.title && (
            <div style={{ fontSize: '11pt', fontWeight: 700, color: accentColor, marginTop: '2px' }}>
              {personal?.title || 'Engineer / Specialist'}
            </div>
          )}
          <div style={{ fontSize: '8.5pt', color: '#64748b', marginTop: '4px', display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
        ...
        {personal?.showPhoto && personal?.photoUrl && (
          <img 
            ...
            style={{ width: '64px', height: '64px', borderRadius: '8px', objectFit: 'cover', border: `2px solid ${accentColor}` }}
          />
        )}
      </header>
          ...
          case 'skills':
            ...
            return (
              <section key="skills" data-section="skills" className="resume-section" style={{ marginBottom: '14px', breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                <h2 style={{ fontSize: '9.5pt', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.04em', color: accentColor, margin: '0 0 6px 0', breakAfter: 'avoid', pageBreakAfter: 'avoid' }}>
                  Skills
                </h2>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
                  {skills.map((s, idx) => {
                    ...
                    return (
                      <span key={s.id || idx} style={{ fontSize: '8.5pt', padding: '2px 8px', background: '#f1f5f9', color: '#0f172a', borderRadius: '4px', fontWeight: 600 }}>
          ...
          case 'experience':
            ...
            return (
              <section key="experience" data-section="experience" className="resume-section" style={{ marginBottom: '16px' }}>
                <h2 style={{ fontSize: '9.5pt', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.04em', color: accentColor, margin: '0 0 8px 0', breakAfter: 'avoid', pageBreakAfter: 'avoid' }}>
                  Work History
                </h2>
